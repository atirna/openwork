...
describe("Agent Plugins 1.0.0", () => {
  test("ships a valid OpenWork Connect manifest and MCP configuration", async () => {
    ...
    expect(validateAgentPluginV1Manifest(manifest)).toMatchObject({
      manifest: { name: "openwork-connect" },
      ok: true,
      warnings: [],
    })
    expect(parseAgentPluginV1McpText(mcpText)).toMatchObject({
      entries: [{
        config: {
